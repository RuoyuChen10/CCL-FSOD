...
def main(
        config_file="configs/lvis/ft.yaml", 
        rpn_config_file="configs/RPN/mask_rcnn_R_50_FPN_1x.yaml",
        model_path="output/train3/lvis_bg1.0/model_0000999.pth", 

        image_dir_root='datasets/lvis/test_dir', 
        output_dir_root='visualization/lvis_bg1.0/ours_result_999', 
        category_space=None,
        device='cuda',
    ...
    config.DE.TOPK = topk
    config.MODEL.MASK_ON = False    # no mask
    ...
    pretrain = torch.load(model_path, map_location=device)['model']
    # del pretrain["train_class_weight"]
    # del pretrain["test_class_weight"]
    model.load_state_dict(pretrain, False)
    model.eval()
    ...
    # p1, p2 = torch.load("weights/initial/few-shot/prototypes/fs_coco14_base_train.vitl14.pkl"), torch.load("weights/initial/few-shot/prototypes/fs_coco_trainval_novel_30shot.vitl14.pkl")
    # model.test_class_weight = torch.cat([p1['prototypes'], p2['prototypes']], dim=0).to(device)
    # model.label_names = p1['label_names'] + p2['label_names']    
    
    # label_names =  model.label_names
    label_names = MetadataCatalog.get('lvis_v1_train').thing_classes
    # if 'mini soccer' in label_names: # for YCB
    #     label_names = list_replace(label_names, old='mini soccer', new='ball')
    
    categories = os.listdir(image_dir_root)
    for category_name in tqdm(categories):
        if category_name!="mallet":
            continue
        image_dir = os.path.join(image_dir_root, category_name)
        output_dir = os.path.join(output_dir_root, category_name)
        ...
        for img_file in glob(osp.join(image_dir, '*')):
            try:
                ...
                colors = assign_colors(pred_classes, label_names, seed=3)
                output = to_pil_image(
                        draw_bounding_boxes(
                ...
                print("Error, file name: {}".format(img_file))

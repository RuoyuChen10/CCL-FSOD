    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        if self.few_shot_mode:
            for iouThr in coco_eval.params.iouThrs:
                ...
                if len(self.seen_cnames) == 20: # pascal voc
                    unseen_cids = [p.catIds[i] for i, c in enumerate(class_names) if c in self.seen_cnames[15:]]
                    seen_cids = [p.catIds[i] for i, c in enumerate(class_names) if c in self.seen_cnames[:15]]
                    all_cids = [p.catIds[i] for i, c in enumerate(class_names) if c in self.all_cnames]
                elif len(self.seen_cnames) != 80:
                    ...
                    novel_categories = ["person", "bicycle", "car", "motorcycle", "airplane", "bus", "train", "boat"
                                        "bird", "cat", "dog", "horse", "sheep", "cow", "bottle", "chair", "couch",
                                        "potted plant", "dining table", "tv"]
                    base_categories = ['truck', 'traffic light', 'fire hydrant', 'stop sign', 'parking meter', 
                                       ...
                                       'pizza', 'donut', 'cake', 'bed', 'toilet', 'laptop', 'mouse', 'remote', 
                                       'keyboard', 'cell phone', 'microwave', 'oven', 'toaster', 'sink', 'refrigerator', 
                                       'book', 'clock', 'vase', 'scissors', 'teddy bear', 'hair drier', 'toothbrush']
                    unseen_cids = [p.catIds[i] for i, c in enumerate(class_names) if c in novel_categories]
                    seen_cids = [p.catIds[i] for i, c in enumerate(class_names) if c in base_categories]
                    all_cids = [p.catIds[i] for i, c in enumerate(class_names) if c in self.all_cnames]

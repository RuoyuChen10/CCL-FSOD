...
from .MemoryBank import MemoryPrototypeBank
from .KGC import ContrastiveHead, KnowledgeGuidedContrastiveLoss
...
class OpenSetDetectorWithKnowledge(nn.Module):
    ...
    def __init__(self,
        ...
        print("This is our meta arch.")
        print("Number of training classes: {}".format(self.num_train_classes))
        print("Number of all classes: {}".format(self.num_classes))
        ...
        self.MemoryPrototypeBank = MemoryPrototypeBank(
            class_num = self.num_train_classes,
            shots = shots,
            shots_param = shots_param)
        self.contrast_head = ContrastiveHead(dim_in = 128, feat_dim = feat_dim)
        
        # New Loss Function KGC
        self.kgc_loss = KnowledgeGuidedContrastiveLoss(
            knowledge_martix_path = knowledge_matrix_path,
    ...
    def from_config(cls, cfg, use_bn=False):
        ...
        # Get the shots
        shots = cfg.KGC.SHOTS
        shots_param = cfg.KGC.CONTAINER
        feat_dim = cfg.KGC.FEATURE_DIM
        knowledge_matrix_path = cfg.KGC.KNOWLEDGE_MATRIX
        coefficient = cfg.KGC.COEF
        tau = cfg.KGC.TAU
        ...
        return {
            ...
            "shots_param": shots_param,
            "feat_dim": feat_dim,
            "knowledge_matrix_path": knowledge_matrix_path,
            "coefficient": coefficient,
            "tau": tau
        }
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        with torch.no_grad():   # ResNet-50, used for RPN.
        ...
        with torch.no_grad():   # DINO V2 ViT
        ...
        if self.training or self.use_one_shot: 
            with torch.no_grad():
                ...
                ## New add ##
                # prototype_gt_classes = [x.gt_classes[0].cpu().item() for x in gt_instances]
                prototype_gt_classes = []
                for x in gt_instances:
                    for x_gt_class in x.gt_classes:
                        prototype_gt_classes.append(x_gt_class.cpu().item())

                prototype_rois = []
                for bid, box in enumerate(gt_boxes):
                    batch_index = torch.full((len(box), 1), fill_value=float(bid)).to(self.device) 
                    prototype_rois.append(torch.cat([batch_index, box], dim=1))
                prototype_rois = torch.cat(prototype_rois)
                ## End add ##
        ...
        if self.training:
            roi_features_prototype = self.roi_align(patch_tokens, prototype_rois) # N, C, k, k
            roi_prototype_bs = len(roi_features_prototype)
            # print(roi_features_prototype.shape)   torch.Size([8, 1024, 7, 7])
            # print(prototype_gt_classes)           [8, 162, 160, 165, 64, 14, 140, 32]
        
        # len(class_labels)     1024
        # roi_features.shape    torch.Size([1024, 1024, 7, 7])

        # roi_features # N x emb x spatial
        #%% #! Classification
        if (self.training and (not self.only_train_mask)) or (not self.training):
            roi_features = roi_features.flatten(2)  # torch.Size([1024, 1024, 49])
            bs, spatial_size = roi_features.shape[0], roi_features.shape[-1]
            # (N x spatial x emb) @ (emb x class) = N x spatial x class
            feats = roi_features.transpose(-2, -1) @ class_weights.T    # torch.Size([1024, 49, 200])

            ### New add ###
            if self.training:
                roi_features_prototype = roi_features_prototype.flatten(2)
                feats_prototype = roi_features_prototype.transpose(-2, -1) @ class_weights.T
            ### End add ###

            # sample topk classes
            class_topk = self.num_sample_class  # In config DE.TOPK
            class_indices = None
            if class_topk < 0:
            ...
            if sample_class_enabled:
                ...
                # init_scores: torch.Size([1024, 200])
                ...
                if self.training:
                    ...
                    for i in range(roi_bs):
                        ...
                        gt_class_indices.append(
                            torch.as_tensor([curr_label])
                        )
                        
                        # 如果topk中的ground truth的类别，则直接取topk的类别
                        if curr_label in topk_class_indices_i or curr_label == num_classes:
                            curr_indices = topk_class_indices_i
                        # 如果topk中无ground truth的类别，则把最后一个删掉，加入ground truth类别
                        else:
                    ...
                    gt_class_indices = torch.stack(gt_class_indices).to(self.device)
                else:
                    class_indices = topk_class_indices
                class_indices = torch.sort(class_indices, dim=1).values # 按选中的类别顺序来，而不是topk顺序
            else:
            ...
            intra_feats = torch.gather(feats, 2, class_indices[:, None, :].repeat(1, spatial_size, 1)) if sample_class_enabled else feats   # 选中的topk个类中有效的
            ...
            intra_dist_emb = self.fc_intra_class(intra_dist_emb)    # torch.Size([50176, 6, 128])
            intra_dist_emb = intra_dist_emb.reshape(bs, spatial_size, num_active_classes, -1)   # torch.Size([1024, 49, 6, 128])
            
            # 这里是对比学习的地方
            if self.training:
                # Proposal feature
                intra_dist_emb_target = intra_dist_emb
                
                index = torch.argmax((class_indices==gt_class_indices).int(), dim=1)
                index_expanded = index.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1, intra_dist_emb_target.shape[1], 1, intra_dist_emb_target.shape[-1])
                intra_dist_emb_target = torch.gather(intra_dist_emb_target, 2, index_expanded).squeeze(2)

                intra_dist_emb_target = intra_dist_emb_target.mean(dim=1)
                target_gt_classes = gt_class_indices.squeeze(1)
                
                intra_dist_emb_target = intra_dist_emb_target[  # torch.Size([145, 128])
                    torch.nonzero(target_gt_classes!=num_classes).squeeze()]
                intra_dist_emb_target = self.contrast_head(intra_dist_emb_target)   # torch.Size([145, 128])
                target_gt_classes = target_gt_classes[torch.nonzero(target_gt_classes!=num_classes).squeeze()]  # torch.Size([145])

                # Prototype feature
                prototype_gt_classes = torch.as_tensor(prototype_gt_classes).to(self.device).unsqueeze(-1)
                
                with torch.no_grad():
                    prototype_feats = torch.gather(feats_prototype, 2, prototype_gt_classes[:, None, :].repeat(1, spatial_size, 1))
                    prototype_emb = distance_embed(prototype_feats.flatten(0, 1), num_pos_feats=self.Tpos_emb) # (Nxspatial) x class x emb TODO: 可以再加一个 linear 层这里
                    prototype_emb = self.fc_intra_class(prototype_emb)
                    prototype_emb = prototype_emb.reshape(roi_prototype_bs, spatial_size, 1, -1)   # torch.Size([8, 49, 1, 128])
                    prototype_emb = prototype_emb.squeeze(2).mean(1)    # torch.Size([8, 128])
                    
                    prototype_emb = self.contrast_head(prototype_emb)
                    prototype_gt_classes = prototype_gt_classes.squeeze(-1).cpu().numpy()
                    
                self.MemoryPrototypeBank.update(prototype_emb, prototype_gt_classes)     # update the store features
                prototype_centers, prototype_classes = self.MemoryPrototypeBank.cluster()  # torch.Size([8, 128])   torch.Size([8])
            ...
            # (Nxclasses) x EMB x S x S torch.Size([6144 (1024*topk), 384, 7, 7])
        ...
        if self.training:
            ...
            ##### New Add #####
            loss_dict['loss_contrast'] = self.kgc_loss(prototype_centers, prototype_classes, intra_dist_emb_target, target_gt_classes)
            ##### End Add #####

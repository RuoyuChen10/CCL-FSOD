...
class StandardROIHeads(ROIHeads):
    ...
    def _init_box_head(self, cfg):
        ...
        self.nums_class_prototype = cfg.BANK_CONTAINER
        ...
        # build prototype memory bank
        self.MemoryPrototypeBank = MemoryPrototypeBank(
            class_num = self.num_classes,
            shots = cfg.SHOT,
            shots_param = self.nums_class_prototype)
        
        # self.contrast_head = ContrastiveHead(dim_in = 1024, feat_dim = 128)
        self.contrast_head = torch.nn.Linear(1024,128)
        
        nn.init.normal_(self.contrast_head.weight, std=0.01)
        nn.init.constant_(self.contrast_head.bias, 0)
        ...
        if cfg.COUNTERFACTUAL.OPERATOR:
            self.counter_number = cfg.COUNTERFACTUAL.COUNTER_NUMBER
            self.explainer = CounterFactual(
                knowledge_matrix = self.knowledge_matrix, 
                counter_number = cfg.COUNTERFACTUAL.COUNTER_NUMBER)
            
        # Loss weight
        self.lambda1 = cfg.LAMBDA1   # cls loss
        self.lambda2 = cfg.LAMBDA2   # reg loss
        self.lambda3 = cfg.LAMBDA3   # contrastive loss
        
        self.contrast_norm = cfg.CONTRAST_NORM
    
    def forward_with_gt_boxes(self, features, targets):
        """for computing counterfactual saliency map
        ...
        gt_features_input = self.box_pooler(    # Prototype features from the gt boxes
            features_list, [x.gt_boxes for x in targets]
        )
        gt_features_input.requires_grad_()
        
        gt_features = self.box_head(gt_features_input, self.wish)
        
        gt_classes = torch.IntTensor([]).to(gt_features_input.device)
        for x in targets:
            gt_classes = torch.cat((gt_classes, x.gt_classes), 0)
        gt_classes = gt_classes.cpu().numpy()
        
        # Prediction
        pred_class_logits, _ = self.box_predictor(
            gt_features
        )   # [batch, class_num]
        
        cam, counter_classes = self.explainer.saliency_map(gt_features_input, pred_class_logits, gt_classes)
        
        return cam, counter_classes
        
    def _extract_prototype_feature(self, features, targets):
        ...
        with torch.no_grad():
            # Stop gradient
            prototype_features = self.box_pooler(    # Prototype features from the gt boxes
                features, [x.gt_boxes for x in targets]
            )
            prototype_features = self.box_head(prototype_features, self.wish)
            prototype_features = self.contrast_head(    # [BS, FEATURE_DIM]
                prototype_features.detach())    # stop gradient from here

            gt_classes = torch.IntTensor([]).to(prototype_features.device)
            for x in targets:
                gt_classes = torch.cat((gt_classes, x.gt_classes), 0)
    ...
    def _extract_proposal_feature(self, proposals, feature_pooled):
        ...
        # keep = (proposal_ious > 0.6)
        # print(keep.shape)
        # print(object_features.shape)
        
        return object_features, object_labels, proposal_ious

    def forward(self, images, features, proposals, targets=None, enhanced_features = None, update = False):
        """
        See :class:`ROIHeads.forward`.
        """
        del images
        ...
        if self.training:
            losses = self._forward_box(features_list, proposals, targets, fg_scale, update)
            return proposals, losses
        else:
            pred_instances = self._forward_box(features_list, proposals)
            return pred_instances, {}

    def _forward_box(self, features, proposals, targets=None, fg_scale=1, update = False):
        """
        ...
        box_features = self.box_pooler(      #ROI池化层
            features, proposal_boxes      # features 为1张图的feature，proposal 16个，每个proposal有256个
        )
        ...
        if self.training:
            object_features, object_labels, proposal_ious = self._extract_proposal_feature(proposals, box_features)
            if update:
                # Prototype Embeddings
                prototype_features, prototype_classes = self._extract_prototype_feature(features, targets)
                # Update Memory Bank
                self.MemoryPrototypeBank.update(prototype_features, prototype_classes)     # update the store features
            # Cluster
            if self.cluster:
                # Prototype Cluster Operation
                prototype_features, prototype_classes = self.MemoryPrototypeBank.cluster()
            else:
                prototype_features, prototype_classes = self.MemoryPrototypeBank.take_off()     # This new function hasn't been debug.
        else:
        ...
        outputs = FastRCNNOutputs(
            ...
            object_features =object_features,       # proposal经过bedding后特征
            object_labels = object_labels,             # proposal的label
            prototype_features = prototype_features,    # 类中心经过bedding后的特征
            prototype_classes = prototype_classes,                  # 类中心的label
            proposal_ious = proposal_ious,                # proposal中是物体的索引
            tau = self.tau,
            knowledge_matrix = self.knowledge_matrix,
            num_classes = self.num_classes,
            extract_prototype = self.extract_prototype,

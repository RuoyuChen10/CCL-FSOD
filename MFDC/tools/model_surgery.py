...
def surgery_loop(args, surgery):
    ...
    if args.method == 'remove':
        ...
        if args.memory:
            args.param_name += ['roi_heads.queue_s', 'roi_heads.queue_l', 'roi_heads.queue_ptr', 'roi_heads.queue_full']
            tar_sizes += [TAR_SIZE, TAR_SIZE, TAR_SIZE, TAR_SIZE]
...
def main(args):
    ...
    def surgery(param_name, is_weight, tar_size, ckpt):
        if param_name in ['roi_heads.queue_s', 'roi_heads.queue_l', 'roi_heads.queue_ptr', 'roi_heads.queue_full']:
            if is_weight:
                weight_name = param_name
                pretrained_weight = ckpt['model'][weight_name]
                if param_name == 'roi_heads.queue_s' or param_name == 'roi_heads.queue_l':
                    prev_cls, feat_size1, feat_size2 = pretrained_weight.size()
                    new_weight = torch.zeros((tar_size, feat_size1, feat_size2))
                else:
                ...
                print(weight_name, pretrained_weight.shape, new_weight.shape)
                return
            else:
                return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='coco', choices=['voc', 'coco', "fsod"])
    ...
    parser.add_argument('--memory', action='store_true', help='For memory bank')

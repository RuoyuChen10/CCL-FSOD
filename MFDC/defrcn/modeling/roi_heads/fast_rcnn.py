...
class FastRCNNOutputs(object):
    ...
    def smooth_l1_loss_distill(self, gt_prototype_classes):
        gt_proposal_deltas = self.box2box_transform.get_deltas(
            self.proposals.tensor, self.gt_boxes.tensor
        )
        box_dim = gt_proposal_deltas.size(1)  # 4 or 5
        ...
        loss_box_reg = smooth_l1_loss(
            self.pred_proposal_deltas[fg_inds[:, None]].reshape(len(fg_inds), -1, box_dim),
            gt_proposal_deltas[fg_inds, None, :].repeat(1, bg_class_ind, 1),
            self.smooth_l1_beta,
            reduction="none",
        )
        #print(loss_box_reg.shape)
        gt_prototype_classes = gt_prototype_classes[fg_inds, :-1].unsqueeze(-1).repeat(1, 1, box_dim)
        #print(gt_prototype_classes.shape)
        loss_box_reg = (gt_prototype_classes * loss_box_reg).sum()
        loss_box_reg = loss_box_reg / self.gt_classes.numel()
        return loss_box_reg
...
class KnowledgeGuidedContrastiveLearningFSOD(FastRCNNOutputs):
    """
    Fast R-CNN head with a new branch of KCL.
    """
    def __init__(
        self,
        box2box_transform,
    ...
    def losses(self):
        ...
        return {
            "loss_cls": self.softmax_cross_entropy_loss(),
            "loss_box_reg": self.smooth_l1_loss(),
            # "loss_contrast": self.contrastive_loss(self.prototype_centers, self.prototype_classes, self.object_features, self.object_labels)
            "loss_contrast": self.contrastive_loss(self.prototype_centers, self.prototype_classes, self.object_features, self.object_labels)
        }
...
@ROI_HEADS_OUTPUT_REGISTRY.register()
class DoubleFastRCNNOutputLayers(nn.Module):
    def __init__(
        self, cfg, input_size, num_classes, cls_agnostic_bbox_reg, box_dim=4
    ):
        super(DoubleFastRCNNOutputLayers, self).__init__()
    ...
    def forward(self, x_s, x_l):
        ...
        proposal_deltas = self.bbox_pred(x_l)

        if self._do_cls_dropout:
            x_s = F.dropout(x_s, self._dropout_ratio, training=self.training)
        scores = self.cls_score(x_s)
